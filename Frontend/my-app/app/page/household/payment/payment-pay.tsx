...
import { Check, Download, ShieldCheck, HelpCircle, CheckCircle, ChevronLeft } from "lucide-react";
...
export default function PaymentPay({
  ...
  useEffect(() => {
    ...
    const startPolling = () => {
      ...
      console.log("Fallback: Starting short polling for payment status...");
      pollInterval = setInterval(async () => {
        try {
          ...
          console.error("Error polling payment status fallback:", err);
        }
    ...
    if (typeof window !== "undefined" && !!window.EventSource) {
      ...
      eventSource.addEventListener("payment_status", (event: MessageEvent) => {
        try {
          ...
          console.error("Error parsing SSE payment status:", err);
        }
      });
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start select-none font-sans">
      ...
      <div className="space-y-4">
        <div className="bg-white rounded-3xl border border-slate-200/60 shadow-sm p-6 space-y-6">
          <h4 className="text-[15px] font-extrabold text-slate-800 border-b border-slate-100 pb-3 tracking-tight">สรุปยอดชำระ</h4>

          <div className="space-y-4">
            <div className="flex items-center justify-between text-[13px] font-bold text-slate-400">
              <span>ยอดรวม (1 รายการ)</span>
              <span className="text-slate-700 font-extrabold">{amount.toFixed(2)} ฿</span>
            </div>
            <div className="flex items-center justify-between text-[13px] font-bold text-slate-400 pb-4 border-b border-slate-100">
              <span>ค่าธรรมเนียม</span>
              <span className="text-[#10B981] font-extrabold">ฟรี</span>
            </div>

            {/* Grand Total Price */}
            <div className="flex items-center justify-between py-2">
              <span className="text-[14px] font-bold text-slate-800">ยอดรวมทั้งสิ้น</span>
              <span className="text-2xl font-black text-[#0B409C] tracking-tight">{amount.toFixed(2)} ฿</span>
            </div>
          </div>

          {/* Action Confirm button (Simulation in Dev Mode) */}
          <button 
            onClick={onSimulateSuccess}
            className="w-full flex items-center justify-center gap-2 bg-[#0B409C] hover:bg-[#09337d] text-white font-extrabold py-4 px-6 rounded-2xl shadow-md transition duration-150 transform active:scale-95 cursor-pointer text-[14px]"
          >
            <Check className="w-4 h-4 stroke-[3]" />
            <span>กลับไปที่หน้าเลือกรายการ</span>
          </button>
          
          <p className="text-[11px] text-center text-slate-400 font-bold leading-normal mt-2">
            *กรุณาตรวจสอบยอดชำระก่อนยืนยัน
          </p>
        </div>
